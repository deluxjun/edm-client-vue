  ...
  <div :style="{'max-height': containerHeight, 'min-height': '300px'}" v-loading="isLoading" >
    <div v-if="!hasError" :element-loading-text="workingText">
      ...
      <div v-show="!isLoading">
        <el-checkbox :disabled="!permissionControlable" class="margin-top10" v-model="isInheritance" name="type" @change="handleInheritanceChanged">{{ isInheritance? $t('messages.inherited') : $t('messages.notInherited') }}</el-checkbox>
        <el-card class="box-card" :body-style="cardStyle" v-loading="loadingPermissions" >
          <el-form ref="security" :model="security" label-width="120px" :disabled="!permissionControlable || isInheritance">
            <!-- select security type -->
            <el-radio-group v-model="vSecurityType" @change="handleSecurityChanged">
              ...
              <el-tooltip class="item" effect="dark" :content="$t('messages.security.super')" placement="bottom" :open-delay="1000">
                <el-radio-button :label="'SUPER'" >{{ $t('label.security.super') }}</el-radio-button>
              </el-tooltip>
              <el-tooltip class="item" effect="dark" :content="$t('messages.security.profile')" placement="bottom" :open-delay="1000">
                <el-radio-button :label="'PROFILE'">{{ $t('label.security.profile') }}</el-radio-button>
              </el-tooltip>
              <el-tooltip class="item" effect="dark" :content="$t('messages.security.local')" placement="bottom" :open-delay="1000">
                <el-radio-button :label="'ACL'">{{ $t('label.security.local') }}</el-radio-button>
              </el-tooltip>
            </el-radio-group>

            <!-- security profiles -->
            <div v-if="vSecurityType == 'PROFILE'" class="margin-top10">
              <el-form-item :label="$t('permission.securityclass')">
              ...
              <Permission v-if="permissionLoaded > 0" :permissionList="profilePermissions" :editable="permissionEditable"></Permission>
            </div>
            <Permission ref="permissions" v-if="vSecurityType == 'ACL'" :permissionList="aclPermissions" :editable="permissionEditable"></Permission>
          </el-form>
        </el-card>
        <div v-if="rewritePrivs && rewritePrivs.length > 0">
          <h3 class="h3"><i class='fa fa-arrow-circle-right'></i> {{ $t('label.approvedPrivs') }}</h3>
...
export default {
  ...
  computed: {
    showPermissions() {
      return this.vSecurityType === 'ACL' || this.vSecurityType === 'PROFILE'
    },
    cardStyle() {
    ...
    permissionEditable() {
      if (this.vSecurityType !== 'ACL' || this.isInheritance)
        return false
      if (!this.permissionControlable)
        return false
      return true
    ...
    permissionList() {
      let permissionList
      if (this.vSecurityType === 'PROFILE'){
        return this.profilePermissions
      }
      else if (this.vSecurityType === 'ACL'){
  ...
  methods: {
    ...
    async init() {
      try {
        ...
        this.securityProfiles = await this.getSecurityProfiles()
...
//        this.aclPermissions.push(...this.BaclPermissions)
    ...
    validate(aclRights) {
      if (this.vSecurityType === 'ACL' && aclRights.length < 1) {
        return false
      }
      if (this.vSecurityType === 'PROFILE' && !this.sclassId) {
        return false
      }
      return true
    },
