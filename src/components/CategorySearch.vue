<template>
    <el-input :placeholder="this.$t('label.select_category')" v-model="currentValue" :disabled="true"
        @focus="handleFocus"
        @blur="handleBlur">
        <el-button class='right-line' slot="append" icon="el-icon-search" @click="openCategoryPicker"></el-button>
        <el-button slot="append" icon="el-icon-error" @click="resetinput"></el-button>
    </el-input>
</template>

<script>
import SelectCategoryDialog from "@/views/SelectCategoryDialog";
...
export default {
  ...
  mounted() {
    ...
    이벤트명을 변경함.
    */
    Event.off(EventList.LOAD_TREEDATA_CATEGORY);
    Event.listen(EventList.LOAD_TREEDATA_CATEGORY, this.defaultSetting);
  ...
  methods: {
    ...
    async openCategoryPicker() {
      ...
      if (result) {
        console.log(result);
        this.$emit("input", result.eclassId);
        this.tempName = result.desc;
        this.setCurrentValue(result.desc);
      }
    },
    resetinput() {
      this.currentValue = "";
