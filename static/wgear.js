...
function WGear ( statusCallback, cfg ) {
    ...
    var config = {
        ...
        ip : "127.0.0.1",
        port : { wss : 20173, ws : 20174 , https : 20171, http : 20172 },
        uid : null,
        modules : [],
        autoConnect : true,
        minVersion : 2.0,
        direct : false,
        spreadEventParams : false,
        baseOnly : false,
        platform : 'AnyCPU', // AnyCPU, x86, x64
        status : 'none' // connecting, initializing, open, closing, closed, error
    };
    ...
    if(cfg){
        ...
        if(typeof cfg.baseOnly != 'undefined') {
            config.baseOnly = cfg.baseOnly;
        }
        if(typeof cfg.port != 'undefined') {
            if(typeof cfg.port.wss != 'undefined') {
                config.port.wss = cfg.port.wss;
            }
            if(typeof cfg.port.ws != 'undefined') {
                config.port.ws = cfg.port.ws;
            }
        }
    }
    ...
    if(statusCallback == null || typeof statusCallback == 'undefined') {
    ...
    function setConfig (msg) {
        if(msg) {
            ...
            if(typeof msg.baseOnly != 'undefined') {
                config.baseOnly = msg.baseOnly;
            }
        }
    }
    ...
    function sendConfigMsg (success, fail) {
        _exec(function (r) {
            ...
            fail();
                ...
                browser : browser, baseOnly : config.baseOnly
                }]);
    }
    ...
    var _exec = function (success, fail, type, module, method, args, options, bin) {
        ...
        {
            case 'connecting':
                fail('WGear state is connecting.');
                return;

            case 'initializing':
                if(type != 'Config') {
                    fail('WGear state is initializing.');
                    return ;
                }
                break;
        }
    ...
    callback.setValue = function (success, fail, module, property, value, options) {
        _exec(success, fail, 'SetProperty', module, property, [], options);
    };
    ...
    function executeListener (msg) {
        ...
        if(listeners) {
            for(var i = 0; i < listeners.length; i++) {
                try {
                    if( !!msg.bin ) {
                        if(config.spreadEventParams == false) {
                            listeners[i]({result : msg.result, bin : msg.bin});
                        } else {
    ...
    promise.addStatusListener = function(listener, allclient) {
        return new Promise(function(resolve, reject) {
            _addStatusListener(function(result) {resolve(result);}, function(error) {reject(error);}, listener, allclient);
        });
    };

    callback.addStatusListener = function(success, fail, listener, allclient) {
        _addStatusListener(success, fail, listener, allclient);
    };

    function _addStatusListener(success, fail, listener, allclient) {
        success = success || function() {};
        fail = fail || function() {};
        allclient = allclient || false;
        if(typeof listener == 'function') {
            if(allclient == true) {
                _addEventListener(success, fail, WGearBase, StatusChanged, listener);
            }
            else {
                for(var i = 0; i < status_listener_list.length; i++) {
    ...
    promise.removeStatusListener = function(listener, allclient) {
        return new Promise(function(resolve, reject) {
            _removeStatusListener(function(result) {resolve(result);}, function(error) {reject(error);}, listener, allclient);
        });
    };

    callback.removeStatusListener = function(success, fail, listener, allclient) {
        _removeStatusListener(success, fail, listener, allclient);
    };

    function _removeStatusListener(success, fail, listener, allclient) {
        success = success || function() {};
        fail = fail || function() {};
        listener = listener || null;
        allclient = allclient || false;
        ...
        if(allclient == true) {
            _removeEventListener(success, fail, WGearBase, StatusChanged, listener)
        }
        else {
            if(status_listener_list) {
                for(var i = 0; i < status_listener_list.length; i++) {
    ...
    var getInfo = function() {
        return { version : config.version, secure : config.secure, ip : config.ip, port : config.port, uid : config.uid, modules : config.modules, minVersion : config.minVersion.toFixed(1), status : config.status, project : config.project, baseOnly : config.baseOnly };
    };
